...
class pjHttp
{
        ...
        switch ($this->getMethod())
        {
            case 'POST':
                ...
                if ($post_fields && is_array($post_fields))
                {
                    ...
                    self::fixFileUploads($post_fields);
                }
    ...
    /**
     * Set the maximum amount of HTTP redirections to follow
     *
     * @param int $value
     * @return pjHttp
     */
    ...
    public function storeAsFile($value)
    {
        if (is_resource($value))
        {
            $this->fp = $value;
        } elseif (is_string($value)) {
            $this->fp = @fopen($value, "wb+");
        }
        
        return $this;
    }
    
    protected static function fixFileUploads(&$data)
    {
        ...
        foreach ($data as $k => $v)
        {
            ...
            foreach ($tmp as $item)
            {
                if (strpos($item, '@') === 0)
                {
                    list(,$file['filename']) = explode('@', $item, 2);
                } elseif (strpos($item, 'type=') === 0) {
                    list(,$file['mimetype']) = explode('=', $item, 2);
                } elseif (strpos($item, 'filename=') === 0) {
                    list(,$file['postname']) = explode('=', $item, 2);
                }
            }
            $data[$k] = curl_file_create($file['filename'], @$file['mimetype'], @$file['postname']);
        }
    }
}