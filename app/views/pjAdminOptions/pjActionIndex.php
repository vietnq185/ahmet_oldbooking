				...
				for ($i = 0; $i < $count; $i++)
				{
					...
					<tr class="pj-table-row-odd<?php echo $rowClass; ?>" style="<?php echo $rowStyle; ?>">
						...
						<td>
							...
							{
								case 'string':
									if (in_array($tpl['arr'][$i]['key'], array('o_site_url', 'o_link_to_inquiry_form'))):
										?>
										<?php
										foreach ($tpl['lp_arr'] as $v)
										{
											?>
											<span class="pj-multilang-wrap" data-index="<?php echo $v['id']; ?>" style="display: <?php echo (int) $v['is_default'] === 0 ? 'none' : NULL; ?>">
												<span class="inline_block">
													<input type="text" name="i18n[<?php echo $v['id']; ?>][<?php echo $tpl['arr'][$i]['key']; ?>]" class="pj-form-field w300<?php echo (int) $v['is_default'] === 0 ? NULL : ' required'; ?>" lang="<?php echo $v['id']; ?>" value="<?php echo htmlspecialchars(stripslashes(@$tpl['arr']['i18n'][$v['id']][$tpl['arr'][$i]['key']])); ?>" />
													<?php if ((int) $tpl['option_arr']['o_multi_lang'] === 1 && count($tpl['lp_arr']) > 1) : ?>
								...
								case 'enum':
									...
									{
										if($tpl['arr'][$i]['key'] == 'o_date_format')
										{
											$_label = substr($enumLabels[$k], 0, 5);
											$label = $_label . ' (' . pjUtil::formatDate(date('Y-m-d'), 'Y-m-d', $_label) . ')';
											if ($default[1] == $el)
											{
												?><option value="<?php echo $default[0].'::'.$el; ?>" selected="selected"><?php echo array_key_exists($k, $enumLabels) ? stripslashes($label) : stripslashes($el); ?></option><?php
											} else {
												?><option value="<?php echo $default[0].'::'.$el; ?>"><?php echo array_key_exists($k, $enumLabels) ? stripslashes($label) : stripslashes($el); ?></option><?php
											}
										}else{
											if ($default[1] == $el)
											{
