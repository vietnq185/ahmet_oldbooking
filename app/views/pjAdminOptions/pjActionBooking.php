		...
		if (is_array($tpl['arr']))
		{
			...
			if ($count > 0)
			{
				...
				for ($i = 0; $i < $count; $i++)
				{
					...
					if (in_array($tpl['arr'][$i]['key'], array('o_creditcard_later_fee')))
					{
					    $rowClass = " boxCreditcardLater";
					    $rowStyle = "display: none";
					    if ($tpl['option_arr']['o_allow_creditcard_later'] == 'Yes')
					    {
					        $rowStyle = NULL;
					    }
					}
					if (in_array($tpl['arr'][$i]['key'], array('o_saferpay_username', 'o_saferpay_password', 'o_saferpay_customer_id', 'o_saferpay_terminal_id', 'o_saferpay_fee')))
					{
						$rowClass = " boxSaferpay";
						$rowStyle = "display: none";
						if ($tpl['option_arr']['o_allow_saferpay'] == 'Yes')
						{
							$rowStyle = NULL;
						}
					}
							...
							switch ($tpl['arr'][$i]['type'])
							{
								...
								case 'float':
									if($tpl['arr'][$i]['key'] == 'o_booking_price'){ 
										...
										<?php
									} elseif($tpl['arr'][$i]['key'] == 'o_creditcard_later_fee' || $tpl['arr'][$i]['key'] == 'o_saferpay_fee')
									{
									    ?>
										<span class="pj-form-field-custom pj-form-field-custom-before">
											<span class="pj-form-field-before"><abbr class="pj-form-field-icon-text">%</abbr></span>
