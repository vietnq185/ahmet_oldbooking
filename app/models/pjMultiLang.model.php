...
class pjMultiLangModel extends pjAppModel
{
	...
	public function getIsoMultiLang($foreign_id, $model)
	{
		$arr = array();
		$_arr = $this->reset()->select('t1.*, t2.language_iso')
			->join('pjLocale', 't2.id=t1.locale', 'left outer')
			->where('foreign_id', $foreign_id)->where('model', $model)
			->orderBy('t2.sort ASC')
			->findAll()->getData();
		foreach ($_arr as $_k => $_v)
		{
			$arr[$_v['language_iso']][$_v['field']] = $_v['content'];
		}
		return $arr;
	}
