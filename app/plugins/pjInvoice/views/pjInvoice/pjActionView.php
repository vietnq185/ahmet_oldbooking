...
<html>
	...
	<body>
		...
		if ($tpl['arr']['status'] == 'not_paid' && (int) $tpl['config_arr']['p_accept_payments'] === 1 && (
			...
			(int) $tpl['config_arr']['p_accept_bambora'] === 1 || (int) $tpl['config_arr']['p_accept_creditcard'] === 1 ||
            (int) $tpl['config_arr']['p_accept_cash'] === 1 || (int) $tpl['config_arr']['p_accept_bank'] === 1
		) && (float) $tpl['arr']['total'] > 0)
		{
			...
			<form id="frmInvoicePayment" action="<?php echo PJ_INSTALL_URL; ?>index.php?controller=pjInvoice&amp;action=pjActionPayment" method="post">
				...
				if ((int) $tpl['config_arr']['p_accept_bambora'] === 1)
				{
					?><p><label><input type="radio" name="payment_method" value="bambora"<?php echo $tpl['arr']['payment_method']=='bambora' ? ' checked="checked"' : null; ?> /> <?php __('plugin_invoice_pay_with_bambora'); ?></label></p><?php
				}
