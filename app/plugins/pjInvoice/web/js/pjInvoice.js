...
(function ($, undefined) {
	...
	$(function () {
			...
			var $grid = $("#grid").datagrid({
				buttons: [{type: "edit", url: "index.php?controller=pjInvoice&action=pjActionUpdate&id={:id}", title: "Edit"},
				          ...
				          {type: "menu", url: "#", text: '', items:[
									 ...
									 {text: myLabel.email_invoice, url: "index.php?controller=pjInvoice&action=pjActionSend&id={:uuid}&uuid={:order_id}", linkClass: 'plugin_invoice_email'}
                                ]
				          }
						 ],
				columns: [
		...
		}).on("change", "input[name='p_accept_paypal'], input[name='p_accept_bambora'], input[name='p_accept_authorize'], input[name='p_accept_bank']", function () {
			var $this = $(this);
			if ($this.is(":checked")) {
				$($this.data("box")).show();
			} else {
