...
class pjAdminEmailThemes extends pjAdmin
{
	public function pjActionCreate()
	{
		...
		if ($this->isAdmin())
		{
			...
			{
				$data = array();
				$id = pjEmailThemeModel::factory($_POST)->insert()->getInsertId();
				...
				pjUtil::redirect($_SERVER['PHP_SELF'] . "?controller=pjAdminEmailThemes&action=pjActionIndex&err=$err");
			} else {
				...
				$this->appendJs('pjAdminEmailThemes.js');
			}
		} else {
			$this->set('status', 2);
		}
	...
	public function pjActionUpdate()
	{
		...
		if ($this->isAdmin())
		{
			if (isset($_POST['action_update']))
			{
				...
				pjUtil::redirect(PJ_INSTALL_URL . "index.php?controller=pjAdminEmailThemes&action=pjActionIndex&err=AET01");
				
			} else {
				$arr = pjEmailThemeModel::factory()->find($_GET['id'])->getData();
				if (count($arr) === 0)
				{
					pjUtil::redirect(PJ_INSTALL_URL. "index.php?controller=pjAdminEmailThemes&action=pjActionIndex&err=AET08");
				}
