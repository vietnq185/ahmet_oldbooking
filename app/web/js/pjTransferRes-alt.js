	...
	TransferRes.prototype = {
		...
		init: function (opts) {
			...
			this.$container.on("click.tr", ".btn", function (e) {
                ...
                pjQ.$("#trCCData_" + self.opts.index + ', .trCartDeposit_Checkout_' + self.opts.index + ', .trCartRest_Checkout_' + self.opts.index).toggle(pjQ.$(this).val() == 'creditcard');
            ...
            pjQ.$(document).on("click", ".trigger", function (e) {
                pjQ.$(this).parent().parent().nextAll('.information').slideToggle(500);
		...
		getLocations: function(){
			...
			pjQ.$.get([this.opts.folder, "index.php?controller=pjFront&action=pjActionGetLocations", "&session_id=", self.opts.session_id].join(""), params).done(function (data) {
                ...
                pjQ.$('#dropoffBox_' + index + ' select').select2({ width: "100%" });
                self.enableButtons.call(self);
